...
#' @param sx,fx,Mx Vectors containing numeric values of the age specific female survival and fertility rates and net migration counts.
#'
#' If \code{fccp_net0} is used a single vector is required.
#'
#' If \code{fccp_net} is used a matrix of period specific rates (or counts for net migration) is required, where rows represent age groups and columns future periods. If a single vector is passed to \code{fccp_net} a matrix based on constant assumptions in all future rates and net migration counts will be constructed.
...
#'
#' If \code{fccp_net} is used a vector of period specific values is required. If a single value is passed to \code{fccp_net} a vector based on constant assumptions in all future rates will be constructed.
...
#'
#' \code{fccp_net0} produces population projections based strictly on constant future rates.
#'
