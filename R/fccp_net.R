...
#' \deqn{ \mathbf N(t+5) =  \mathbf L \left( \mathbf N(t) + \frac{1}{2} \mathbf M \right) + \frac{1}{2} \mathbf M   }
...
#' @param sx,fx,Mx Vector containing numeric values of the age specific female survival and fertility rates and net migration counts.
#'
#' If \code{fccp_net0} is used a single vector is required.
#'
#' If \code{fccp_net} is used a matrix of period specific rates (or counts for net migraiton) is required, where rows represent age groups and columns future periods. If a single vector is be passed to \code{fccp_net} a matrix based on constant assumptions in all future rates and net migration counts will be constructed.
#' @param sn,sex_ratio Numeric value of the survivorship of newborn female babies from birth to the end of the interval and the sex ratio at birth of newborn babies.
#'
#' If \code{fccp_net0} is used a single value is required.
#'
#' If \code{fccp_net} is used a vector of period specific values is required. If a single value is be passed to \code{fccp_net} a vector based on constant assumptions in all future rates will be constructed.
...
#'
#' \code{fccp_net0} produces population projections based stricly on constant future rates.
#'
#' \code{fccp_net} produces population projections based non-constant future rates.
#'
#' @export
#'
#' @examples
...
#'          year0 = 1993, age_lab = df0$age,
#'          tidy_output = FALSE)
fccp_net0 <- function(n = NULL, x = df0$x, p = NULL, Nx = NULL,
                      sx = NULL,
  ...
  if(length(Mx) != xx)
    stop("Mx must be of the same length as x")
...
#' @export
#' @rdname fccp_net0
fccp_net <- function(n = NULL, x = df0$x, p = NULL, Nx = NULL,
                     sx = NULL,
                     fx = NULL, sn = NULL, sex_ratio = 1/(1 + 1.05),
                     Mx = NULL,
                     tidy_output = TRUE, age_lab = x, gender_lab = "Female", ...){
  ...
  if(nrow(Mx) != xx | ncol(fx) != n)
    stop("Mx must have the same number of rows as x and the same number of columns as n")
  if(length(sn) != n)
  ...
  for(i in 1:n){
    ...
    L[xx, xx] <- sx[xx, i]
    L[1, 1:xx] <- p * sn[i] * sex_ratio * 0.5 * (fx[,i] + lead(fx[,i]) * sx[,i])
    L[is.na(L)] <- 0
