#' Simple Projection Model Based on Dynamic Crude Demographic Rates and Net Migration Counts.
#'
#' A simple projection model based on dynamic crude demographic rates and net migration counts,
...
#'
#' If \code{sp_net} is used used a vector of period specific rates (and net migration counts) is required. If a single value is be passed to \code{sp_net} a vector based on constant assumptions in all future rates (and net migration counts) will be constructed.
